...
if($_GET[player]==1||$_GET[player]==2){
	if(is_numeric($_GET[game])){
		mysql_query('UPDATE `games` SET `score'.$_GET[player].'`=1+`score'.$_GET[player].'` WHERE `id`="'.mysql_real_escape_string($_GET[game]).'" AND `score'.$_GET[player].'`<10');
		if(mysql_affected_rows()==1){
			// Now mark game as completed if 10 goals have been scored
			mysql_query('UPDATE `games` SET `gameOver`="1", `timeOver`="'.time().'" WHERE `id`="'.mysql_real_escape_string($_GET[game]).'" AND `score'.$_GET[player].'`="10"');
			if(mysql_affected_rows()==1){ // Update the ELOs of the players since the game is over
				// Get the IDs of the players
				$idsQuery = mysql_query('SELECT `player1`, `player2`, `score1`, `score2` FROM `games` WHERE `id`="'.$_GET[game].'" LIMIT 1');
				...
				$newELO = $rating1+($k*((($_GET[player]==1)?1:0)-(1/(1+pow(10, (($rating2-$rating1)/400))))));
				mysql_query('UPDATE `users` SET 
												`rating`="'.$newELO.'", 
												`gamesWon`=`gamesWon`+'.(($_GET[player]==1)?1:0).', 
												`gamesLost`=`gamesLost`+'.(($_GET[player]==1)?0:1).',
				...
				$newELO = $rating2+($k*((($_GET[player]==2)?1:0)-(1/(1+pow(10, (($rating1-$rating2)/400))))));
				mysql_query('UPDATE `users` SET 
												`rating`="'.$newELO.'", 
												`gamesWon`=`gamesWon`+'.(($_GET[player]==2)?1:0).', 
												`gamesLost`=`gamesLost`+'.(($_GET[player]==2)?0:1).',
